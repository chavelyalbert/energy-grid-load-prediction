...
# DBTITLE 1,Compute hourly means. According to dataset info: The actual generation shall be computed as the average of all available instantaneous Net generation output values on each market time unit.
print(generation.schema["index"].dataType)
...
generation_hourly.write.format("delta").mode("overwrite").saveAsTable(
    "workspace.schema_capstone.generation_clean"
)
